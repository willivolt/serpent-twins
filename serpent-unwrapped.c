...
int start_x, start_y;
double orbit_angle = 180.0; // camera orbit angle, degrees
double camera_elevation = 0; // camera elevation angle, degrees
double camera_distance = 8.0; // metres
...
#define BLUR_BRIGHTNESS_SCALE 0.22
...
vector compute_spine_point(int segment, double fraction) {
  ...
  result.y = 0;//sin(4*t/TOTAL_LENGTH)*2;  // a gentle curve
  result.z = 0;
  return result;
}
...
vector compute_cylinder_point(
  ...
  vector upward = multiply((double)a/na*3.141-3.141/2, up);  // assume seam on bottom
  vector rightward = multiply(sin(angle/2)*RADIUS*0.2, right);
...
void update_camera() {
  ...
  glLoadIdentity();
  gluPerspective(FOV_DEGREES, camera_aspect, 1, 1e2); // fov, aspect, zrange
